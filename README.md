...
* 🚦 محدودیت سرعت اختصاصی برای هر کانفیگ + دکمه «حداکثر سرعت همه» برای برداشتن یک‌جای تمام محدودیت‌ها
* ⚡ موتور عبور پُرسرعت: بافر ۱MB، سوکت ۴MB، ثبت مصرف batch، WebSocket بدون فشرده‌سازی اضافه و رفع قفل Token Bucket روی چانک‌های بزرگ
...
* **ALPN چندانتخابی**: انتخاب هم‌زمان `h3`، `h2` و `http/1.1`، پروفایل‌های آماده و نمایش ALPN مؤثر. برای جلوگیری از قطعی، خروجی WS خودکار روی `http/1.1` و XHTTP روی مقادیر سازگار `h2,http/1.1` امن‌سازی می‌شود؛ `h3` برای ترابردهای آینده ذخیره می‌ماند.
...
در منوی پنل دو صفحه مستقل **«بکاپ و بازیابی»** و **«آموزش انتقال پنل»** اضافه شده است. صفحه آموزش، ساخت سرویس جدید از GitHub، Volume، Restore تلگرام، انتقال DNS و تست نهایی را کاملاً مرحله‌به‌مرحله توضیح می‌دهد تا با تنظیمات اصلی قاطی نشود.

توکن ربات، آیدی ادمین‌ها، چت مقصد، فاصله بکاپ و بازیابی خودکار را می‌توانید مستقیماً در صفحه بکاپ تنظیم و اتصال را تست کنید؛ بنابراین تنظیم متغیرهای تلگرام از Railway اجباری نیست. تنظیمات در فایل خصوصی `telegram_settings.json` داخل `DATA_DIR` ذخیره می‌شوند.

پس از Restore، دو کادر مستقل برای **دامنه ثابت ساب** و **دامنه Railway کانفیگ‌ها** وجود دارد. ذخیره این دو کادر فوراً داخل برنامه اعمال و در `/data/app_settings.json` ماندگار می‌شود. بخش اختیاری «همگام‌سازی Railway Variables» نیز با Railway API Token، لینک صفحه Variables سرویس را می‌خواند و `PUBLIC_BASE_URL` و `CONFIG_PUBLIC_HOST` را با API رسمی Railway ثبت می‌کند. توکن Railway در فایل خصوصی `/data/railway_settings.json` با دسترسی محدود ذخیره می‌شود و عمداً داخل بکاپ تلگرام قرار نمی‌گیرد.
...
## 🌐 اگر ساب بدون VPN آپدیت نمی‌شود

این مشکل معمولاً از فیلترشدن IPهای Proxy دامنه است، نه از فرمت ساب. در Cloudflare رکورد دامنه ثابت ساب را روی **DNS only (ابر خاکستری)** قرار دهید و Proxy نارنجی را خاموش کنید. پنل برای هر کانفیگ و گروه یک «ساب مستقیم Railway» اضطراری نیز نمایش می‌دهد. پاسخ ساب با `no-store`، CORS عمومی و فاصله آپدیت یک‌ساعته ارسال می‌شود تا کلاینت‌ها نسخه قدیمی را از Cache نگیرند.
